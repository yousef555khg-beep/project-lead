...
| PL-27 | Every objective receives a fresh execution model | One executor finishes a small Spark objective, then receives a complex objective in the same task while Spark capacity may also be unavailable. | Ask once per project for routing authority; classify every objective again; use Spark only when every allowlist condition is true; otherwise select Terra; never inherit the current model or weaken review routing. | The pre-change core had no execution-model section. The historical candidate added reset, eligibility, fallback, and review separation; its switching assumption is superseded by PL-36 and its effort defaults by PL-38. | Pass (historical; routing details superseded) |
...
The PL-36 GREEN candidate records model, reasoning effort, and service tier per dispatch, starts formal user-visible tasks fresh, gives internal helper and reviewer tasks no or bounded history, verifies atomically exposed routes or uses a no-project-access handshake, reports an unobservable route as `blocked_on_routing`, keeps fallback blocking local to one scope, and sends a non-blocking route notice. Its original effort defaults are superseded by PL-38's current-child calibration.
...
## PL-37: sidebar-visible formal executor tasks

The RED case was a live Shengxue Youpin controller dispatch: formal work ran as an internal subagent, so it did not appear as a user-owned sidebar task. The user could not find its full history or an approval card by browsing the task list.

The repair distinguishes work by responsibility, not by model. Formal implementation, independent review, and long validation create a titled user-visible task through `create_thread`. Internal subagents remain available only for short read-only helper checks and cannot own mutable scope, wait for approval, review, accept, or finish formal work. A missing visible-task creator becomes `blocked_on_visibility`; it never silently downgrades to an invisible dispatch.

The first RED tests could not find a titled visible-task contract or public explanation. The final regressions require both, require formal `create_thread` tasks to start fresh, and bind the complete core to its reviewed digest. The evidence is structural: it verifies the written routing boundary, not that a future platform will always expose `create_thread`.

## PL-38: task-local model and effort calibration

The RED replay exposed an inverted incentive: the old rule permitted Spark `xhigh` whenever every Low-risk condition held, so a one-file `tsconfig` include repair with one deterministic `typecheck` was over-routed. Live controller evidence also showed a minimal typecheck cleanup using Terra `xhigh`, while a separate multi-source license, identity, and schema reconciliation had enough interacting constraints to justify Terra `xhigh`.
