...
- Executor results were treated as candidates. The controller checked scope and evidence, then required independent review before acceptance.
...
**Observed value.** The workflow kept one module's green result from being presented as approval for the whole product. Review findings and unresolved release gates remained visible until they had their own evidence.
...
- Candidates were checked for scope, branch and commit identity, and fresh verification before independent review.
...
以上两个案例分别来自私有的渔迹 iPhone/Apple Watch 产品，以及盛学优品小程序与商家平台。公开内容只描述项目治理方法：按模块划分责任、复用兼容任务、保存任务句柄、区分候选结果与最终验收、要求独立审查，并对鉴权、部署和外部操作设置权限边界。本文不包含任何私有代码、接口、路径、账号、密钥、客户数据或未发布功能。
