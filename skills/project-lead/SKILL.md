...
5. Record a control ledger: objective; module to task; Base/Head; scope; dependencies; status; cursor; model override; review verdict. Keep it in context/plan unless the user requests a file.

## Govern to acceptance

1. Use event-driven `wait_threads` or `wait_agent` with current cursors. Never create timers, recurring automation, heartbeat, or periodic polling.
2. User interruption: answer in commentary, then resume unless it cancels or replaces work.
3. On completion, verify branch, Base/Head, scope, clean status, and fresh checks. Executor self-report is not acceptance.
4. **REQUIRED SUB-SKILL:** invoke `requesting-code-review` with exact requirements and Base/Head. Critical or Important findings return to the same executor; wait for a new candidate and review the changed diff again.
5. **REQUIRED SUB-SKILL:** use `verification-before-completion` before reporting.
6. Send final only when every required task is accepted, cancelled, or blocked on user action. Report changes, files, checks, limits, next step, and release recommendation.

If the controller process terminates, proactive return is impossible. On the next turn, recover from recorded handles and resume; never conceal this with automation.
