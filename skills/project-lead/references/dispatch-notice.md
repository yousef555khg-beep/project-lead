...
Show this Markdown table in commentary immediately before each new task or substantive follow-up (execution, required review, or assistance), never inside a code fence. Fill actual task values, not the placeholders or an example's default model/effort.

| 项目 | 内容 |
| --- | --- |
| 执行任务 | <本次具体目标，一句话> |
| 任务线程 | <真实任务名称；尚未创建时用计划名称，不编造链接> |
| 模型 | <此次明确选择的模型 ID> |
| 推理档 | <此次明确选择的档位> |
...
Do not repeat unchanged tables during waiting or factual peer clarification, and do not duplicate the old one-line notice alongside the table. Existing explicit approval and ownership boundaries still apply.
